  ...
  renderProductInfo() {
    fetch(`${this.dataset.url}?variant=${this.currentVariant.id}&section_id=${this.dataset.section}`)
      ...
      .then((responseText) => {
        ...
        const priceUpdate = formatMoney(this.currentVariant.price, "${{amount}}")
        const salePrice = this.currentVariant.compare_at_price

        if (salePrice) {

          const salePriceFormatted = formatMoney(salePrice, "${{amount}}")

          price.innerHTML = `<div class="price__element-sale">
          <span class="sr-only">Sale price</span>
          ${priceUpdate}<s class="text-red-700">${salePriceFormatted}</s>
        </div>`
        } else {
          price.innerHTML = `<div class="price__element-normal">
